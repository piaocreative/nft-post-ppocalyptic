        ...
        <div className="mint-ticket border border-white mb-5">
            ...
            <div className="d-flex gap-4 mt-5 justify-content-center flex-wrap">
                ...
                <div className="d-flex justify-content-center gap-3">
                    <button onClick={ setDown } className="fs-4 change-number">-</button>
                    <input type="number" className="text-center" value={item} onChange={ e => setItem(e.target.value) } />
                    <button onClick={ setUp } className="fs-4 change-number">+</button>
                </div>
                <button className="btn rounded-pull" onClick={onMintPressed}>Mint Now</button>
            </div>
            {status}
        </div>
    )
